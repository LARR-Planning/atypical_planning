...
public:
    ...
    bool update(bool log) override {
        Environment mapf(dimx, dimy, dimz, ecbs_obstacles, ecbs_goalLocations, std::vector<double>{param.agent_xy_size}, param.grid_xy_res);
...
private:
    ...
    bool isObstacleInBox(const std::vector<double>& box, double margin){
        for(octomap::OcTree::leaf_bbx_iterator it = octree_obj->begin_leafs_bbx(octomap::point3d(box[0], box[1], box[2]),
    ...
    bool setWaypoints(){
        ...
        xig = (int)round((mission.startState[0] - grid_x_min) / param.grid_xy_res);
        yig = (int)round((mission.startState[1] - grid_y_min) / param.grid_xy_res);
//        zig = (int)round((mission.startState[2] - grid_z_min) / param.grid_z_res);
        zig = 0;
        xfg = (int)round((mission.goalState[0] - grid_x_min) / param.grid_xy_res);
        yfg = (int)round((mission.goalState[1] - grid_y_min) / param.grid_xy_res);
